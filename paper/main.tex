...
%%% AUTHOR : Khushi (@your-github)
%%% -----------------------------------------------------

\subsection{Adversarial ML Threats and Defenses}

Recent research shows that artificial intelligence is becoming central to 6G because it helps future wireless networks adapt more quickly, automate decision-making, and manage increasingly complex environments. At the same time, this shift expands the security problem: attackers can now target not only network infrastructure, but also the machine-learning models that guide network behavior. Broad adversarial machine learning surveys show that evasion and poisoning attacks remain major concerns, while 6G-focused work argues that AI-enabled optimization introduces new risks in areas such as access control, resource allocation, and autonomous network management. As a result, security in 6G must be studied as both a networking problem and a learning-system problem.
...
This paper proposes a layered framework that secures 6G AI systems across three stages: data, language model, and decision making.
