...
The shift from 5G to 6G is a major change in how we think about staying connected. While 5G improved throughput, latency, and network virtualization, 6G expands connectivity into integrated terrestrial, aerial, and satellite networks that allow 3D coverage (Kaur and Gupta 2025). By connecting ground stations with satellite networks, 6G aims to reduce or eliminate coverage gaps (Catak et al. 2021). This creates a global network where devices can switch between ground towers and satellites in space. To make this possible, research visions for 6G propose speeds approaching 1 Tbps and support for up to 10^6 IoT devices per km² (Kaur and Gupta 2025).

To build such a large network, the industry is moving toward Open Radio Access Network (O-RAN). In the past, network gear was tightly integrated, meaning the hardware and software had to come from the same company. O-RAN attempts to standardize interfaces so the technology is more universally compatible (Kaur and Gupta 2025). This allows providers to mix and match hardware and software from different vendors. In this open system, AI helps run the network through controllers: xApps handle fast tasks such as routing signals and optimizing resources, while rApps manage longer-term network tasks (Kaur and Gupta 2025).

\subsection{The Shift from Perimeter Security to Zero Trust}

\subsection{AML Threats and Explainable AI}
